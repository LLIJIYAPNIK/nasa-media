...
def build_digest_router(
    ...
    async def cmd_digest(callback_query: CallbackQuery) -> None:
        ...
        await message.answer(
            ...
            reply_markup=get_digest_kb(user.digest_subscribed, user.weekly_highlights_subscribed),
        )
    ...
    @router.callback_query(F.data == "weekly_highlights_show")
    async def weekly_highlights_show(callback_query: CallbackQuery) -> None:
        message = require_message(callback_query)
        async with ChatActionSender.typing(bot=require_bot(message), chat_id=message.chat.id):
            await deliver_weekly_highlights.execute(week_start(date.today()), message.chat.id)

    register_subscribe_handlers(router, MediaSourceKind.DIGEST, set_subscription)
    register_subscribe_handlers(router, MediaSourceKind.WEEKLY_HIGHLIGHTS, set_subscription)
