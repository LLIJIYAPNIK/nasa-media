...
def _router(deliver_media: AsyncMock, get_or_create_user: AsyncMock, set_birthday: AsyncMock) -> Router:
...
async def test_cosmic_birthday_asks_for_date_when_not_set_yet():
    ...
    router = _router(deliver_media, get_or_create_user, AsyncMock())
...
async def test_finish_birthday_saves_and_delivers_when_within_apod_bounds():
    ...
    deliver_media = AsyncMock()
    router = _router(deliver_media, AsyncMock(), set_birthday)
