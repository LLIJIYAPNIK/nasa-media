...
def _photo_message(message_id: int, file_id: str) -> MagicMock:
    return MagicMock(message_id=message_id, photo=[MagicMock(file_id=f"{file_id}-small"), MagicMock(file_id=file_id)])


async def test_publish_single_sends_by_direct_url_when_telegram_accepts_it():
    bot = MagicMock()
    bot.send_photo = AsyncMock(return_value=_photo_message(42, "file-42"))
...
async def test_publish_single_falls_back_to_download_when_telegram_rejects_direct_url():
    bot = MagicMock()
    bot.send_photo = AsyncMock(side_effect=[TelegramBadRequest(MagicMock(), "bad url"), _photo_message(7, "file-7")])
    ...
    assert ref.file_id == "file-7"
    assert bot.send_photo.await_count == 2


async def test_publish_animation_sends_gif_and_returns_message_id():
    bot = MagicMock()
    bot.send_animation = AsyncMock(return_value=MagicMock(message_id=77, animation=MagicMock(file_id="gif-file-77")))
    gateway = TelegramAdminChatGateway(FakeClientSession({}), bot, ADMIN_CHAT_ID)
    ...
    assert ref.file_id == "gif-file-77"
...
async def test_publish_generated_image_sends_photo_and_returns_message_id():
    bot = MagicMock()
    bot.send_photo = AsyncMock(return_value=_photo_message(88, "file-88"))
    gateway = TelegramAdminChatGateway(FakeClientSession({}), bot, ADMIN_CHAT_ID)
